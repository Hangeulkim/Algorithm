...
21/12/22
이진 검색 트리 - 단순 구현에 대한 귀찮음이 굉장히 심해진 것 같다. 다시 조금 더 바쁘게 살아야 할 것 같다.

21/12/27
행렬 제곱 - 수학 관련 문제는 단위 행렬같은 간단한 지식을 까먹어서 푸는데 오래걸리는 경우가 많은 것 같다. 주기적으로 다시 풀어보는게 필요할 것 같다.

21/12/28
플로이드 - 단순 플로이드 와샬 구현 문제

21/12/29
피보나치 수 6 - 행렬 수학 문제 피보나치 수를 행렬로 변환하여 접근할 수 있다는 걸 알게되었다. 수열을 행렬로 접근하여 푸는 방식도 항상 염두에 두고 풀어야 할 것 같다.

2022
22/01/03
구간 합 구하기 5 - 아이디어는 빠르게 떠올랐으나 마지막 변수 순서에서 막힌 문제 이상하다 싶을때는 예제를 잘 살펴보자

22/01/04
최소비용 구하기 2 - 다익스트라에 이전 방문 경로를 저장하는 방식으로 해결하는 문제 이전에 공부했었던 방문경로 저장방법이 기억나 쉽게 풀었던 것 같다.

22/01/05
시그마 - 수학 구현문제는 다 만들고 나서 공식 중 한 부분이 잘못 됐다는 걸 찾는게 힘든 것 같다. 처음부터 실수하지 말자

22/01/07
서강그라운드 - 단순 플로이드 와샬 알고리즘 문제

22/01/10
A -> B - 간단한 bfs 구현 문제

22/01/11
파이프 옮기기 1 - 단순 구현 탐색 문제
미세먼지 안녕 - 단순 구현 탐색 문제

22/01/14
잃어버린 괄호, IOIOI, 나는야 포켓몬~ - 단순 구현 문제
종이의 개수, 쿼드트리, 색종이 만들기 - 단순 dfs 탐색 구현 문제

22/01/17
ATM, 좌표 압축, 이중 우선순위 큐, 패션왕 신해빈 - 단순 수학 구현 문제
경로 찾기 - 길찾기 문제
비밀번호 찾기 - map 이용 문제
Four Squares - 수학 dp 문제

22/01/18
벡터 매칭 - 단순 수학 구현 문제
최소 스패닝 트리 - mst 우선순위 큐 프림 알고리즘 이용 문제

22/01/26
팰린드롬 분할 - 팰린드롬을 구하는 것 까지는 쉬웠으나 그걸 dp로 구현하는게 어려웠던 문제
</pre>
